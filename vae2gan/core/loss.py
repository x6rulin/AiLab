...
def D_wgan(G, D, latents, reals, eps=1e-3):
    fake_images_out = G(latents)
...
def D_wgan_gp(G, D, latents, reals, gp_lambda=10.0, gp_target=1.0, eps=1e-3):
    fake_images_out = G(latents)
    real_scores_out = D(reals)
    fake_scores_out = D(fake_images_out)
    loss = fake_scores_out - real_scores_out
...
def D_hinge_gp(G, D, latents, reals, gp_lambda=10.0, gp_target=1.0):
    fake_images_out = G(latents)
    real_scores_out = D(reals)
    fake_scores_out = D(fake_images_out)
    loss = torch.relu(1. + fake_scores_out) + torch.relu(1. - real_scores_out)
...
def G_logistic_saturating(G, D, latents):
    fake_images_out = G(latents)
    fake_scores_out = D(fake_images_out)
...
def G_logistic_nonsaturating(G, D, latents):
    fake_images_out = G(latents)
    fake_scores_out = D(fake_images_out)
...
def D_logistic(G, D, latents, reals):
    fake_images_out = G(latents)
    real_scores_out = D(reals)
    fake_scores_out = D(fake_images_out)
    loss = -torch.log(1 - torch.sigmoid(fake_scores_out)) # softplus(fake_scores_out)
...
def D_logistic_simplegp(G, D, latents, reals, r1_gamma=10.0, r2_gamma=0.0):
    fake_images_out = G(latents)
    real_scores_out = D(reals)
    fake_scores_out = D(fake_images_out)
    loss = -torch.log(1 - torch.sigmoid(fake_scores_out)) # softplus(fake_scores_out)
...
def G_loss(G, D, latents, key: str = 'wgan'):
    if key == 'wgan':
        return G_wgan(G, D, latents)
    if key == 'saturating':
        return G_logistic_saturating(G, D, latents)
    if key == 'nonsaturating':
        return G_logistic_nonsaturating(G, D, latents)

    raise RuntimeError(f"Not surpports loss function: '{key}'")


def D_loss(G, D, latents, reals, key: str = 'wgan', eps: float = 1e-3,
           gp_lambda: float = 10.0, gp_target: float = 1.0,
           r1_gamma: float = 10.0, r2_gamma: float = 0.0):
    if key == 'wgan':
        return D_wgan(G, D, latents, reals, eps)
    if key == 'wgan_gp':
        return D_wgan_gp(G, D, latents, reals, gp_lambda, gp_target)
    if key == 'hinge':
        return D_hinge(G, D, latents, reals)
    if key == 'hinge_gp':
        return D_hinge_gp(G, D, latents, reals, gp_lambda, gp_target)
    if key == 'logistic':
        return D_logistic(G, D, latents, reals)
    if key == 'logistic_simplegp':
        return D_logistic_simplegp(G, D, latents, reals, r1_gamma, r2_gamma)
